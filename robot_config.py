...
# ===============================
# 이동 / 스윕 속도   ★전부 미실측 — 실물 준비되면 여기부터 채울 것
# ===============================
# [문제] 같은 물리량이 파일마다 다른 값으로 박혀 있고 자동 동기화가 없다.
#   FOCAL_PX 나 FIRE_TEMP_C 처럼 여기로 모아야 하는데, 아직 어느 쪽도
#   실측값이 아니라서 "무엇을 골라야 하는지" 를 정할 근거가 없다. 그래서
#   지금은 **재야 할 목록**으로만 둔다 — 실측 후 아래 값을 채우고, 그때
#   각 소비처가 여기를 import 하도록 바꿀 것.
#
#   ★소비처를 지금 바꾸지 말 것: 시뮬 값을 건드리면 그동안 쌓은 A/B
#     측정(경로 스무딩 등)이 전부 무의미해진다. 실측 -> 값 확정 -> 배선
#     순서를 지킬 것.
#
#   물리량        | 시뮬(path_planner_sim2) | 실기(motor_commander) | 펌웨어(.ino)
#   -------------|-------------------------|-----------------------|-------------
#   직진 속도     | ROBOT_SPEED_MPS 0.15    | mm*4  -> 0.25 m/s     | -
#   회전 속도     | ROBOT_TURN_DPS  75      | deg*10 -> 100 deg/s   | -
#   스윕 각속도   | SERVO_SWEEP_DPS 100     | -                     | 5도/60ms -> <=83.3
#   ToF 측정주기  | TOF_PERIOD_S 0.020(50Hz)| -                     | 스텝당 1회 -> ~16.7Hz
#   서보 가동범위 | SERVO_MAX_OFF   65      | -                     | SWEEP_*_OFF ±80
#
#   ★서보 범위는 이 파일이 이미 ±80(ESP32_SERVO_*_OFF)으로 정해 두었는데
#     시뮬만 65 를 쓴다. 시뮬 주석은 "(robot_config 서보범위)" 라고 적어
#     놓고 값은 안 맞는 상태다 — 실측이 아니라 **배선만 고치면 되는 건**
#     이것 하나다.

# --- 실측 절차 (직진/회전 명령 계수) --------------------------------------
#   duration_ms = distance_mm * K + C   형태로 잡는다. 원점을 지나는 단순
#   비례식(mm*4)은 잘 안 맞는다 — 정지마찰 극복 + 가속 구간 때문에 절편 C
#   가 생기고, 짧은 이동일수록 그 비중이 커진다.
#     1. speed 값 고정(100 이 PWM 인지 먼저 확인 — 바뀌면 전부 무효)
#     2. duration 100/200/400/800ms 를 각 5회씩, 실제 이동거리를 자로 측정
#     3. (duration, 거리) 직선 피팅 -> 기울기 K, 절편 C
#     4. 바닥재가 바뀌면 재측정 (매끈한 실내 != 잔해 깔린 현장)
#   회전도 같은 절차. 단 회전은 자이로(yaw)가 있어 폐루프 검증이 가능하다.
#
#   ★같은 실험에서 '횡방향 벗어남'도 같이 잴 것 — SMOOTH_MAX_MM 상한의
#     근거가 된다. 어차피 직선으로 여러 거리를 굴려볼 거라 추가 비용이 없다.
#     5. 각 거리마다 도착점이 목표 직선에서 좌우로 몇 mm 벗어났는지 측정
#     6. 거리 -> 횡오차 곡선을 그린다
#     7. 오차가 로봇 몸통 마진의 절반(= ROBOT_WIDTH_CELLS//2 * CELL_SIZE / 2
#        = 2칸 * 50mm / 2 = 50mm)에 닿는 거리가 실물 기준 상한이다
#   [왜 필요한가] segment_safe() 가 검사하는 건 '직선'인데, 좌우 바퀴
#   속도차 때문에 실제 궤적은 '호'다. 그 차이가 몸통 마진을 갉아먹는다.
#   시뮬은 포즈가 곧 진실이라 이 오차가 아예 없어서, 시뮬만 보고 정한
#   SMOOTH_MAX_MM 은 실물에서 과대평가일 수 있다.

# 경로 스무딩 상한. 현재 값은 **시뮬 20시드 A/B 로만** 정한 것이며 실물
# 미검증이다 — 위 5~7 절차로 횡오차를 재면 실물 기준 상한이 나온다.
# 시뮬 근거(SERVO_MAX_OFF=65 기준 20시드 스윕): 충돌 0 = 100/200/250mm,
# 충돌 발생 = 300mm(1시드) / 400mm(7시드) / 800mm(5시드). 안전 경계는
# 250~300 사이였다.
# ★2026-08-27 SERVO_MAX_OFF 를 65->80 로 올린 뒤(실물 SG90 확인, 이 파일
#   윗쪽 "이동/스윕 속도" 절 참고) 같은 200mm 로 재검증하니 20시드 중
#   1개에서 충돌이 났다 - 스캔 커버리지가 넓어지며 탐사 순서/스무딩 타이밍
#   상호작용이 바뀐 것으로 보인다. 200 이 여유가 얇았다는 신호로 보고
#   150 으로 낮췄다. 노이즈일 수도 있어 정밀 재스윕은 아직 안 했다.
SMOOTH_MAX_MM = 150.0   # ★실물 미검증 (path_planner_sim2 가 자체 사본을 씀)

MS_PER_MM = 4.0         # TODO: 미측정 — motor_commander.move_forward() 가 여기서 직접 가져다 씀(단일출처)
MOVE_ACCEL_MS = 0.0     # TODO: 미측정 — 위 식의 절편 C, 동상

# ★MS_PER_DEG 는 더 이상 쓰이지 않는다 (2026-08-27) — 실제 이동제어
#   펌웨어(main_base.ino)를 대조해보니 회전은 duration_ms 를 안 보고
#   자이로(MPU6050 DMP) 폐루프로 고정 90도까지 스스로 돈다. "몇 도를
#   몇 ms 로 번역" 하는 문제 자체가 회전 쪽에서는 사라졌다 - 남은 건
#   "임의 각도가 필요할 때 고정 90도를 어떻게 깨느냐" 쪽이다(아래 참고).
#   값은 과거 실험과의 비교를 위해 지우지 않고 남겨만 둔다.
MS_PER_DEG = 10.0       # 미사용(참고용) — 회전은 이제 시간이 아니라 자이로 상태 명령

# ★회전은 고정 90도만 지원한다(main_base.ino 의 LEFT_TURN_ANGLE/
#   RIGHT_TURN_ANGLE 이 상수). 경로 스무딩(SMOOTH_MAX_MM)이 만드는 임의
#   각도(18.43도 등)는 실물에서 전부 90도로 잘린다 - motor_commander.py
#   상단 주석 참고. 고칠 때 후보 두 가지:
#     (a) .ino 의 두 상수를 함수 인자로 바꿔 임의각 회전을 지원
#     (b) startTurn90() 을 아예 안 쓰고, 직진 PID(updateStraightControl)
#         의 targetYaw 를 원하는 각도로 바로 세팅 - 그쪽은 이미 임의
#         각도를 받는다(제자리 회전 전용 함수만 90도로 막혀있을 뿐).
#         새 펌웨어 파라미터 없이 될 가능성이 있어 (a) 보다 가볍다.
#   ★둘 다 지금은 미착수 - "원본 .ino 변수 안 건드림" 방침으로 보류.

# --- 오도메트리 (엔코더 하드웨어 미완성) ----------------------------------
#   착수 절차/차동구동 공식/한계는 odometry_plan.md 참고.
#   ★엔코더가 붙으면 위 MS_PER_* 번역 자체가 사라진다(거리로 직접 명령).
MM_PER_TICK = None      # TODO: 미측정 — 엔코더 틱 1개당 이동거리(mm)
WHEEL_BASE_MM = None    # TODO: 미측정 — 좌우 바퀴 간격. 회전각 산출에 필요
